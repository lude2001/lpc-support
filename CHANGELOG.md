...
- 恢复迁移前的宏语言能力：Rust 常驻索引现在提供对象宏与函数式宏的语义高亮、定义跳转、带相邻普通注释/Javadoc 及源码链接的悬浮和参数 snippet 补全，并识别项目配置中的预定义宏；全局 include、直接 include 与嵌套 include 的宏环境会缓存并参与条件编译，头文件变化会清缓存并重建已打开文档及反向依赖。整行函数式宏（包括来自 include 的宏）生成的变量和函数声明会进入文档符号、补全、悬浮与跳转，连续的全大写顶层宏调用也不会吞掉后续声明。宏索引复用依赖图缓存，避免高频请求反复扫描工作区。宏有效性遵守条件编译、重定义和 `#undef` 的源码顺序，不再高亮或解析非活动分支及已失效的宏。补全能力重新声明 `#`、引号、`<` 和 `/` 触发字符，输入预处理指令及 include/inherit 路径时会自动弹出候选。
