...
- 标准 efun 文档改为构建时从 `mud.wiki` 抓取并固化到插件内置资源。
- 运行时不再联网更新标准 efun 文档，补全和悬停直接读取离线数据。
- 保留模拟函数库的本地目录扫描能力。
...
- 新增配置项 `lpc.enforceLocalVariableDeclarationAtBlockStart`（默认 `false`）。
- “局部变量定义必须在可执行语句或代码块的开头”规则改为可选开关。
- 兼容 FluffOS 新版本在代码块任意位置声明局部变量的行为，默认不再提示该错误。
