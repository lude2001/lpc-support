...
- 对象成员推断支持宏路径、`load_object`/`clone_object`、`model_get`、`@lpc-return-objects`、局部变量传播，以及 `lpc-support.json` 配置的实例解析函数；静态字符串局部别名也可继续传入对象构造函数或 `model_get`，不再中断后续方法的补全、跳转和悬浮。
