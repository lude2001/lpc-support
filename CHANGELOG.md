...
- 对象成员推断支持宏路径、`load_object`/`clone_object`、`model_get`、`@lpc-return-objects`、局部变量传播，以及 `lpc-support.json` 配置的实例解析函数；静态字符串别名、局部赋值和可证明的包装函数返回值会继续传播，包装函数的首个静态字符串实参也能绑定到对象构造调用。
