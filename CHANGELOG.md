...
- bundled efun 目录与当前 FluffOS 官方源码重新对齐：补齐 Promise/async、引用环、FFI、debug 和 WebAssembly JS bridge 共 35 个已有官方文档的 efun，保留具名参数、重载、返回类型及包/平台可用条件；移除当前官方驱动已不存在的 `mapping_origin_stats`。同步器和严格审计现锁定 446/446 个 `.spec` efun，缺失、过宽与过窄均为 0。
- efun 签名帮助恢复按当前参数位置选择重载：输入第二或第三个参数时不再固定显示第一条签名，尾随可变参数会稳定停留在 collector 参数上。
- 函数签名改由 Rust CST 直接生成结构化参数事实，函数文档中心不再从签名字符串反推类型、数组层级、`ref`、默认值或尾随 `...`。参数数量模型按 FluffOS 驱动语义区分普通默认参数与 `varargs`：函数级 `varargs` 和 true-varargs 都接受 `0..∞` 个实参，默认参数则降低普通函数的最小实参数量。
- 函数 Javadoc 改由 Rust 索引一次性解析为结构化文档，悬浮、签名帮助、补全与函数文档中心共享摘要、逐参数说明、返回值、详情、备注和返回对象；兼容 type-first、name-first、无类型参数、指针与多行标签，并保留未知标签和文档质量问题。跨文件实现缺少文档时会按匹配原型回退，但定义跳转仍指向实现；`@file` 文件头不会误附着到首个函数，条件编译指令可位于文档与声明之间，普通 include 则保持附着边界。
- 修复签名帮助协议声明：输入后续参数的逗号现在会重新触发签名帮助并刷新当前参数位置，与迁移前行为一致。
- 恢复迁移前的宏语言能力：Rust 常驻索引现在提供对象宏与函数式宏的语义高亮、定义跳转、带相邻普通注释/Javadoc 及源码链接的悬浮和参数 snippet 补全，并识别项目配置中的预定义宏；全局 include、直接 include 与嵌套 include 的宏环境会缓存并参与条件编译，头文件变化会清缓存并重建已打开文档及反向依赖。整行函数式宏（包括来自 include 的宏）生成的变量和函数声明会进入文档符号、补全、悬浮与跳转，连续的全大写顶层宏调用也不会吞掉后续声明。函数宏名称不再被带空格的参数列表截断，多行反斜杠续行宏会保留完整逻辑值；未 include 的头文件宏不再产生高亮、悬浮、跳转或补全误报。宏索引复用依赖图缓存，避免高频请求反复扫描工作区。宏有效性遵守条件编译、重定义和 `#undef` 的源码顺序，不再高亮或解析非活动分支及已失效的宏。补全能力重新声明 `#`、引号、`<` 和 `/` 触发字符，输入预处理指令及 include/inherit 路径时会自动弹出候选。
- 修复跨文件预处理顺序：头文件现在在实际 `#include` 位置读取调用方当时的宏环境，并把其中的 `#define` / `#undef` 按顺序回流；支持宏形式的 include 路径，头文件条件编译不再被“预先合并全部最终宏”选错分支。导入宏在 include 之前不会被高亮、悬浮、跳转或用于生成声明，include 之后才进入有效范围；上下文缓存只比较头文件实际依赖的宏值，避免为每个源文件复制完整环境。
- 修复 Rust 诊断误把 `object->method(...)`、`objects[index]->method(...)` 的接收者变量当作普通函数调用，真实工作区参数数量误报从 91 条降为 12 条；类型诊断不再把匿名函数的返回值归给外层函数，并接受 LPC 的 `return 0` 空值哨兵及 `mixed` 函数的空返回。
- 恢复迁移前保守的未定义函数与未定义符号诊断：只有依赖完整、不是对象成员、宏、efun、模拟 efun、命名继承或动态 include 上下文时才报告；头文件和被文本包含的 `.c` 片段不会脱离最终宿主产生误报。多变量 `foreach`、匿名函数参数、FluffOS 预定义宏和项目头文件宏现在均能正确识别。
- 恢复迁移前的空 heredoc 多行字符串与文件命名诊断；动态 heredoc 标记和 URI 百分号编码文件名同样按真实内容判断。
...
- 工作区冷索引使用会主动让出 CPU 的单后台线程，降低低性能 CPU 上的持续占用。
- 工作区索引取消现在同时覆盖目录发现和逐文件分析阶段；配置重载、重新索引或语言服务器退出后，旧 generation 不会继续遍历大型 mudlib。
- 宏现在支持跨 include 的查找引用与重命名；重命名会覆盖所属 `#define`、有效引用及对应 `#undef`，并按源码顺序隔离后续重定义和非活动条件分支。
...
- Rust 补全现在按当前标识符前缀和词法作用域过滤，不再混入其他函数的局部变量或整个工作区的无关同前缀符号；未知对象成员补全回到保守的公共方法集合。
- Rust 补全恢复函数参数 snippet，并支持 `efun::`、继承作用域、预处理指令、include/inherit 路径以及 struct/class 字段上下文。
- 命名继承作用域（如 `char::query`）现在只解析到 basename 唯一匹配的直接父类分支，并让跳转、悬浮、签名、补全、引用和重命名共享同一解析结果；同名父类或未知限定符保持无结果，避免跨分支误操作。
- 对象成员推断支持宏路径、`load_object`/`clone_object`、`model_get`、`@lpc-return-objects`、局部变量传播，以及 `lpc-support.json` 配置的实例解析函数；静态字符串别名、局部赋值和可证明的包装函数返回值会继续传播，包装函数的首个静态字符串实参也能绑定到对象构造调用。静态函数数组经单变量 `foreach` 迭代时保留完整候选集合，嵌套在 `catch(...)` 等调用中的对象成员也能正确提取接收者。
- 对象目标文件同时含函数前置声明和实现时，跳转、悬浮与补全优先使用实现，避免重复位置和实现 Javadoc 丢失。
- Rust 现在消费 `config.hell` 同步出的全局 include 与 include 目录；include/inherit 整条声明均可跳转，宏形式的 inherit 同样可解析。
- 跨文件继承函数和可证明对象成员的引用与重命名按解析后的定义身份筛选，避免修改同名局部变量或无关文件中的同名声明。
- 依赖正反向图、路径目标和唯一字符串宏现在按工作区索引缓存并在文件或配置变化时失效；真实大型项目中的高频 `query` 引用查询不再为每个对象接收者重复扫描整个工作区。
- 未知对象的方法名不再凭工作区唯一同名函数生成可能错误的跳转、悬浮或签名信息。
- LSP 静态探针新增脱敏的签名帮助、引用、重命名准备和补全 snippet 检查，只报告数量、文件范围与文档存在性，不记录源码或文档正文。
...
- 优化「全局变量未使用」检查规则：
  - 将 `lpc.enableUnusedGlobalVarCheck` 设为默认关闭，避免在 LPC 高度动态、继承链及跨文件共享变量场景下产生干扰；
  - 自动排除 `.h` 头文件中的全局变量检查，避免公共头文件引入变量时的误报；
  - 检查级别由黄色警告（Warning）调整为弱提示（Hint），并采用浅灰变淡样式标注未使用的变量，不再在“问题”面板中产生多余告警波浪线。

## [0.52.11] - 2026-08-02

### 函数文档中心

- 全面升级函数文档中心：可统一浏览当前文件、继承文件和包含文件中的函数，并清晰查看函数签名、参数、返回值、原型与实现、覆盖关系及文档完整度。
- 新增按函数名、签名、参数和文档内容搜索，以及来源、文档状态和声明类型筛选；标准 Efun 与模拟函数可以按需载入，加载期间会显示明确的进度提示。
- 新增跳转到定义、查找引用和复制签名操作；模拟函数现在可以从搜索结果直接跳转到实际源码定义。
- 新增函数文档骨架生成、缺失内容补全和不一致标签安全修复，可处理重复、过时或无法识别的参数与返回值标签，并保留文件原有换行格式。
- 支持未保存文件、继承或包含文件变化以及项目配置变化后的实时刷新；单个依赖文件分析失败时，仍会保留其他可用的函数文档。
- 修复指针参数（如 `string *parts`）被误判为过时参数或丢失说明、同名声明遗漏、包含来源筛选错误，以及模拟函数文档状态在跳转前后不一致的问题。
- 修复宽屏函数列表无法滚动、窄屏列表与详情滚动相互干扰，以及加载过程中短暂显示“没有匹配的声明”的问题。
- 增强面板更新稳定性，避免快速编辑、连续搜索、切换函数或关闭面板时显示过期内容或出现错误提示。

## [0.52.10] - 2026-07-27

### 条件编译与代码高亮

- 修复由 FluffOS driver 内建宏控制的代码块可能被整段显示为灰色未启用代码的问题。对于已经启用数据库等可选能力的 driver，相关源码现在可以正常高亮，并继续提供悬停、跳转和补全。
- `lpc-support.json` 新增 `preprocessorDefines`，用于声明当前 driver 已启用、但没有写在 `config.hell` 或全局头文件中的编译期宏。请在 mudlib 根目录的 `lpc-support.json` 顶层填写宏名数组，例如 `"preprocessorDefines": ["__PACKAGE_DB__"]`；不要填写 `#define` 或宏值。保存后扩展会自动重新读取配置。
- 此配置适用于 `#ifdef`、`#ifndef` 和 `defined(...)` 条件，只影响扩展的静态分析、高亮、悬停、跳转和补全，不会为 FluffOS 启用对应功能。配置内容应与当前 driver 的实际编译选项保持一致。

### 安全与依赖

- 更新网络请求、路径匹配和文本解析相关依赖，修复当前兼容范围内可直接处理的已知安全问题。
...
- 输入 `var` 等声明修饰符前缀时，补全会优先选中 `varargs` 等 LPC 关键字，而不是同前缀 efun。
- 修复刚输入 `::` 或 `room::` 后还没有方法名前缀时，scoped method 补全不能立即出现的问题。

## [0.52.5] - 2026-07-09

### 补全

- 补全现在会覆盖更多 LPC 关键字与声明修饰符，包括 `varargs`、`private`、`static`、`break`、`sizeof` 等。
- 成员访问、scoped 访问、预处理指令和 include / inherit 路径等场景会通过 LSP 触发字符更主动请求补全。

## [0.52.4] - 2026-07-07

### 类型检查诊断

- 修复 `new(__FILE__)` 等使用 FluffOS 预定义文件路径宏创建对象时，被误报为 `object` 赋值类型不匹配的问题。

## [0.52.3] - 2026-07-03

### LSP 响应与性能

- 减少对象访问诊断、签名帮助、对象方法推断和 scoped method 补全 / 解析路径中的临时候选数组分配。
- 签名帮助和对象方法推断在嵌套调用或链式访问场景中继续优先选择最内层匹配位置，同时降低不必要的候选筛选开销。

## [0.52.2] - 2026-07-03

### LSP 响应与性能

- 内置 efun 文档加载改为并行异步读取，减少 LSP 初始化阶段被顺序文件读取阻塞的时间。
- 优化文件扫描与类型诊断中的部分循环和数组过滤路径，降低大型 mudlib 后台诊断与索引刷新时的额外开销。

## [0.52.1] - 2026-07-02

### 类型检查诊断

- 修复 `query_temp(...) || ({})` 等 LPC fallback 写法被误判为 `int`，导致数组或字符串赋值产生类型不匹配误报的问题。
- 类型检查现在能识别 efun 文档中的 `string | string *`、`void | object *` 等联合类型，避免 `add_action("do_halt", "halt")` 等合法调用被误报为参数类型不匹配。

## [0.52.0] - 2026-07-01

### 类型检查诊断

- 新增保守的类型检查诊断，可提示变量初始化、普通赋值、函数返回值、直接函数调用实参、数组 / mapping 字面量和 class / struct 成员访问中的确定类型不匹配。
- 类型检查会复用当前文件、include、inherit、标准 efun 与 simul-efun 中可确认的类型和函数签名；遇到依赖未解析、宏未展开、动态调用、`mixed` 或 `unknown` 时保持静默，减少误报。
- 新增设置 `lpc.enableTypeChecking`，可关闭 `lpc.type.*` 类型检查诊断。

## [0.51.2] - 2026-06-29

### LSP 响应

- 编辑文件时，补全会更少被后台诊断和缓存刷新阻塞，减少输入变量名或函数名时补全列表出现缓慢的问题。
- 首次打开文件后，诊断刷新会避开最初的交互窗口，降低刚打开大型 mudlib 文件时补全、悬停和语义高亮互相抢占造成的卡顿。

## [0.51.1] - 2026-06-29

### LSP 响应与刷新

- 悬停、补全和跳转定义会更积极复用已完成的分析结果，减少打开真实 mudlib 后反复悬停函数时的等待时间和 CPU 抖动。
- include、inherit 或对象目标文件更新后，相关跳转定义、悬停、补全和诊断会更及时使用最新文件内容，减少保存依赖文件后仍看到旧结果的问题。
- 对象方法补全在继承链较长或索引尚未完整时会先返回当前可确认的候选，并在后台继续补齐索引，降低首轮补全卡顿。
- LSP 静态探针新增性能报告，可查看语义高亮、跳转、悬停、补全和诊断等待阶段的耗时，默认仍保持真实项目路径与源码内容脱敏。
...
- 首次打开 mudlib 时可选择预热 LPC 工作区索引，状态栏会显示索引状态，并可随时点击重建；预热用于提升首次跳转、header owner 识别和诊断刷新体验。
- 工作区索引构建中，状态栏会显示当前已处理文件数与总文件数，便于判断构建进度。
- LSP 现在会跟踪打开文件实际依赖过的 include、inherit 与对象目标文件；这些依赖更新后，相关打开文件的诊断和语言功能会按需刷新，减少保存文件后仍看到旧报错、旧跳转结果的问题。
- 跳转定义等语言功能运行前会先确认当前文件与依赖状态是否过期，必要时重新读取最新内容，避免修改依赖文件后必须重启编辑器或 LSP 才能生效。
- 依赖文件变更后，相关打开文件的诊断会自动排队刷新，降低 include / inherit 文件更新后未定义误报长期停留的概率。
- `.lpc` 文件更新现在也会触发 LSP 刷新，避免这类源文件修改后跳转定义、诊断结果或相关检查继续使用旧状态。
