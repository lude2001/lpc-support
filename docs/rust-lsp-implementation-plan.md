...
2026-09-15 的真实项目差异复核确认 Gate D 仍是发布阻断项：Rust 已修复函数级 `varargs` 参数数量误报、源码函数 Javadoc 丢失、补全跨作用域污染以及未知对象方法的猜测式跳转；但类型可证明的对象成员、`::` 作用域成员、结构体/类成员、include/inherit 路径和预处理上下文补全仍需逐项达到 TypeScript 基线。在这些差异全部进入自动化契约前，不得把“请求有返回”解释为功能等价，也不得以性能数据替代功能验收。
