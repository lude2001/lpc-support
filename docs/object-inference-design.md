...
实例解析（Instance Resolution）是 lpc-support 的核心能力之一，用于在 `obj->method()` 形式的成员访问表达式中，静态解析出 `obj` 所指向的 LPC 对象文件路径，从而为 Definition、Completion、Hover 和 Signature Help 四条 Provider 链提供精确的跳转、补全和文档查询基础。

核心设计原则是：统一的 `ObjectInferenceService` 作为唯一入口，被多个语言服务共享调用，避免各 Provider 各自实现解析逻辑。

自 `0.43.0` 起，`::method()` 与 `room::method()` 这类显式父对象 / inherit 分支调用也已经进入主语言服务链，但它们不走 `ObjectInferenceService` 的 `obj->method()` 接收者解析机制，而是由相邻的 `ScopedMethodResolver` / `ScopedMethodReturnResolver` 负责解析。自 `0.44.0` 起，这条 scoped 链又继续接入了 `ScopedMethodDiscoveryService` / `ScopedMethodCompletionSupport`，从而把 scoped 调用补全也统一收敛到同一套 callable-documentation、definition、hover、signature help 与返回对象传播主路径。`0.45.0` 曾尝试引入工作区级 `references / rename`；自 `0.45.1` 起，导航主路径重新收窄为“当前文件级 + 可证明继承链级”的保守边界，旧的 `Workspace*` relation 栈也已退场，不再属于当前生产导航架构。自 `0.45.2` 起，completion / hover / signature help / navigation / instance resolution / runtime document source 的 owner 进一步统一回到显式 composition root 与默认 factory，历史 fallback / self-assembly 路径已基本清空。自语义求值基础接入后，`ObjectInferenceService` 在 direct/member call receiver 上不再只依赖 ad-hoc builtin/doc 规则，而会先消费 `SemanticEvaluationService` 的自然 return 求值与环境 provider 结果；`@lpc-return-objects` 仍保留，但已经降级为 fallback-only 提示。自 `0.45.4` 起，receiver 表达式本身也可以消费受限语义求值结果，静态路径拼接、可证明逻辑 guard 和 folded `new(...)` 会进入实例解析主链，同时 identifier 可见绑定与代码块作用域仍由 legacy tracing 保护，避免内层局部变量泄漏到外层或污染宏 fallback。自 `0.45.5` 起，scoped method 与对象方法 lookup 明确区分“文档声明范围”和“跳转定位范围”：hover / signature help / completion resolve 使用完整函数范围组装 declaration key，definition 仍定位到函数名 token。
...
- **`ObjectInferenceResult`**：解析结果，包含 `status`（`resolved` / `multiple` / `unknown` / `unsupported`）、`candidates` 列表和可选 `reason`
- **`ClassifiedReceiver`**：分类后的接收者，联合类型，区分 `literal`、`macro`、`identifier`、`call`、`index`、`unsupported` 六种形态
- **`InferredObjectAccess`**：完整的解析上下文，包含 `receiver`（原始文本）、`memberName`（成员名）和 `inference`（解析结果）
...
职责：在实例解析器消费 `@lpc-return-objects` 之前，先尝试用统一的受限语义求值层解析调用表达式返回值。
...
`@lpc-return-objects` 仍然保留，但只作为 fallback-only 提示。只要语义求值层产出非 `unknown` 结果，包括 `non-static`，实例解析器都不会再继续落到注解候选。
...
receiver 表达式语义求值只在可证明时转成对象候选；`unknown` 或无法解析文件路径时继续交给 legacy 实例解析路径。对 identifier receiver，旧的可见绑定和作用域判断仍然是安全护栏：已有候选、`non-static`、诊断或可见但无法证明的绑定不会被冲突的 receiver 语义值覆盖。
...
职责：在当前函数作用域内追踪变量赋值，解析标识符接收者的来源。
...
### 3.7 实例解析服务入口 — `src/objectInference/ObjectInferenceService.ts`

职责：统一编排解析流程，是所有 Provider 调用的唯一入口。

构造参数：

- `macroManager?: MacroManager` — 宏管理器，用于宏展开
- `instanceResolutionFunctions?: Record<string, string[]>` — 按函数名配置运行时函数可能返回的对象路径

核心方法：

- **`inferObjectAccess(document, position)`**：完整解析流程
...
通过环境语义 provider 返回 `lpc-support.json` 中 `instanceResolutionFunctions.this_player` 配置的候选对象。若未配置，则作为运行时动态来源处理。
...
`instanceResolutionFunctions` 可以按函数名配置多个候选对象路径，例如 `environment: ["/inherit/room/room"]`。配置命中时这些对象会参与补全、跳转和悬停；未配置的运行时 object efun 会保守标记为 `non-static`。
...
当 `model_get` 的返回对象可以通过形参绑定、局部变量、静态 mapping registry、简单分支和 `new` / `load_object` 等白区语义证明时，系统会直接使用自然语义求值结果。若自然语义求值成功，`@lpc-return-objects` 不会覆盖该结果。
...
void test() {
    ...
    ob->start();          // 解析为 /adm/daemons/combat_d
}
```
...
void test() {
    ...
    ob->start();          // 解析为 /adm/daemons/health_d
}
```
...
当调用表达式不是内建函数，或自然语义求值在白区外保守降级为 `UnknownValue` 时，`ReturnObjectResolver` 才会从当前文件的 Javadoc 注释中提取 `@lpc-return-objects` 标注：
...
1. 自然语义求值（callee return / registry factory / 白区静态值流）
...
| `unknown` | 无法确定路径（如未识别的宏、动态路径、未配置运行时函数返回对象） |
| `unsupported` | 结构上不可静态解析（如数组索引访问、数字字面量、动态字符串拼接） |
...
2. 若解析结果的 `memberName` 与光标所在单词一致，进入 `handleInferredObjectMethodCall`
...
6. 若有输入前缀，仅返回解析结果；否则合并解析结果与回退候选
...
`::method()` 与 `room::method()` 不属于 `obj->method()` 的对象接收者实例解析，但它们现在与实例解析共享同一套 syntax / semantic / callable-documentation 主路径。
...
**`lpc-support.json` — `instanceResolutionFunctions`**

用于配置运行时函数的实例解析候选。当 `this_player()->method()`、`environment()->method()` 等调用出现时，系统会读取对应函数名下的对象路径数组作为候选。
...
| 测试文件 | 覆盖范围 |
|---------|---------|
| `src/objectInference/__tests__/ObjectInferenceService.test.ts` | 实例解析服务完整流程：字面量、宏、内建调用、变量追踪、块作用域、if/else 合并、控制流降级、传递追踪、文档注释返回对象 — 38 个用例 |
...
| `src/__tests__/providerIntegration.test.ts` | Definition Provider 集成：实例解析与 scoped 返回传播到真实跨文件定义的完整链路验证 |
| `src/__tests__/completionProvider.test.ts` | Completion Provider 集成：解析结果到补全候选的完整链路验证 |
...
- `previous_object()` 等运行时依赖的 efun — 需要调用栈信息，不适合静态解析
...
- 利用 `SemanticSnapshot` 的函数签名信息增强参数实例解析
