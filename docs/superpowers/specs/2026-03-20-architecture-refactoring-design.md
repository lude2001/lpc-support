...
src/formatter/printer/
  ├── FormatPrinter.ts         (~120行) 入口 + 分派 + trivia 处理
...
`normalizeInlineText`、`appendToLastLine`、`trimTrailingWhitespace` 等无状态纯函数提取到 `printerUtils.ts` 或保留原位。
...
将 945 行、7 个 Map、5 个职责的 EfunDocsManager 拆为门面 + 独立子服务。
...
src/efun/
  ...
  ├── FileFunctionDocTracker.ts(~180行) 跟踪当前文件 + 继承文件 + include 文件的函数文档
