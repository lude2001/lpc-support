...
- 复用现有 `ObjectMethodReturnResolver`
...
- 对“已知 receiver 对象候选 + 方法名”的返回对象求解器

当全局对象 initializer 是对象方法调用时，`GlobalObjectBindingResolver` 必须复用现有
`ObjectMethodReturnResolver`，而不是把对象方法返回值解析重新塞进
`ReturnObjectResolver`。
...
当前文档的 `SemanticSnapshot` 必须通过共享快照入口获取。推荐唯一入口为：

- `ASTManager.getSemanticSnapshot(document, false)`

仅在当前快照不可用时，才允许退回到等价的共享 best-available accessor。
...
- 若 initializer 是普通表达式来源：
  - 优先复用现有 `ReturnObjectResolver.resolveExpressionOutcome(...)`
- 若 initializer 是对象方法调用：
  - 必须先解析 receiver 的静态对象候选
  - 再复用现有 `ObjectMethodReturnResolver`
  - 其返回候选、multiple 结果与缺失返回注解诊断，沿用现有对象方法返回传播语义
