...
此外还提供错误树相关命令，包括刷新、清空错误、管理错误服务器、切换服务器、打开错误位置和复制错误信息。
...
- 错误诊断中心为空：检查 `lpc.errorViewer.servers` 配置和服务端错误接口是否可用。它与 `lpc-support.json -> compile.remote.servers` 是两套独立配置
