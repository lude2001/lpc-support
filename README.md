...
LPC Support 是一个面向 VS Code 的 LPC / FluffOS 开发扩展，当前版本为 `0.46.1`。
...
- 已使用 `D:\code\shuiyuzhengfeng_lpc` 做真实 mudlib smoke 验证，覆盖 `message.c`、HTTP controller、条件编译 header 和 10 个 `RequestType(...)` controller
...
扩展会先提供注释、字符串、预处理、关键字、类型、数字、操作符、heredoc 和 array delimiter 等基础颜色；当语言分析完成后，会进一步提供更准确的变量、函数、成员访问、efun 和宏高亮。
