...
- 修复 `SKILL_D(...)->method()` 这类函数式宏对象调用被误报为缺少分号的问题
- 修复局部变量或函数参数与标准 efun 同名时，裸调用悬停和签名帮助错误回退到 efun 文档的问题；显式 `efun::method(...)` 仍会识别为标准 efun 调用，并在语义高亮中保持标准 efun 颜色
- 本地编译支持新的 `lpccp` 编译模式，并能读取连接失败、运行时错误和目录汇总的结构化 JSON 结果
...
- 新增基础语义诊断：直接函数调用参数数量、未定义函数和未定义普通符号会在静态可证明时以 Warning 提示
- 格式化会保留预处理行和行尾注释，缺失 include 的警告不会阻断格式化
- 宏补全、宏悬停、宏定义跳转和语义高亮更准确，不再仅按大写命名猜测宏
- 变量查看面板更准确地展示当前文件变量
- 支持实例解析、当前文件与继承链静态全局对象绑定、跨文件返回对象传播和更准确的成员补全/跳转
...
扩展会基于解析后的 LPC 结构进行变量、声明位置、宏、include、对象访问和基础语义等检查。对于真实 mudlib 中常见但较复杂的写法，诊断会尽量只报告静态可证明的问题，避免把正常可编译代码误报为错误。
...
- 变量和函数参数会遮蔽同名裸 efun 调用，避免普通变量悬停、签名帮助或语义高亮误显示 efun 文档；显式 `efun::method(...)` 不受遮蔽
...
- 宏路径对象
- `this_object()`
- `this_player()`、`environment()` 等配置型运行时函数（需配置 `instanceResolutionFunctions`）
...
- 当前函数内的简单变量赋值链
- `if / else` 分支合并
- 受限函数返回值自然求值，包括静态 registry / mapping、局部别名、简单分支、`new` / `load_object` / `find_object` 返回链
...
- `instanceResolutionFunctions` 只在你希望支持运行时 efun 返回对象实例解析时填写；key 为函数名，value 为该函数可能返回的对象文件路径数组
...
- `instanceResolutionFunctions`
  - 为 `this_player()`、`environment()` 等函数提供静态对象候选，结构为 `{ "函数名": ["对象路径"] }`
- `compile.mode`
  - `local` 或 `remote`
- `compile.local.useSystemCommand`
  - 为 `true` 时直接调用系统中的 `lpccp`
- `compile.local.lpccpPath`
  - 当 `useSystemCommand` 为 `false` 时使用显式 `lpccp` 路径；相对路径按工作区根目录解析
- `compile.local.compileMode`
  - 可选 `reload-loaded`、`compile-only`、`fresh-required`；未设置时按 `lpccp` 默认重载模式处理
...
`compileMode` 用来选择本地编译行为：

- `reload-loaded`：默认模式，目标已加载时尝试销毁并重载
- `compile-only`：只验证编译，不替换已加载对象；目标已加载时会返回明确失败原因
- `fresh-required`：要求目标在当前 runtime 中未加载；已加载时提示重启 runtime 后验证
...
如果你希望支持 `this_player()->method()`、`environment()->method()` 等运行时函数的实例解析，可以在 `lpc-support.json` 中增加：

```json
{
  "instanceResolutionFunctions": {
    "this_player": ["/obj/user"],
    "environment": ["/inherit/room/room"]
  }
}
```

扩展会优先尝试自然语义求值函数返回值和 receiver 表达式值。比如静态 registry / mapping factory、局部变量别名、简单 `if / else` 分支，以及 `new(...)`、`load_object(...)`、`find_object(...)` 这类返回对象来源，都可以在受限白区内自然传播：
...
实例解析会继续保留可见绑定和作用域优先级：内层代码块局部变量不会泄漏到外层，同名宏 fallback 也不会被已离开作用域的局部变量污染。

当自然语义求值无法证明结果时，你仍然可以用 `@lpc-return-objects` 标注自定义函数返回对象实例：
...
`@lpc-return-objects` 现在是 fallback-only 提示：只在自然语义求值和环境语义 provider 都无法给出非 unknown 结果时才会接管。若自然语义求值已经得到确定结果，则以自然语义求值为准。

### efun 实例解析边界

实例解析只在结果可证明或用户显式配置时产出对象候选；某些 efun 虽然返回 `object` 或 `object *`，但结果依赖运行时状态，默认不会被当作静态路径猜测。

可自然静态求值的 efun：

- `this_object()`：解析为当前文件。
- `load_object(path)`：当 `path` 可静态求值为对象路径时解析。
- `find_object(path)`：当 `path` 可静态求值为对象路径时解析。
- `clone_object(path)`：当 `path` 可静态求值为对象路径时解析，并可在函数返回和变量追踪中继续传播。
- `master()`：从 `config.hell` / `config.dev` 的 `master file` 读取主控对象路径。

可通过 `instanceResolutionFunctions` 配置候选对象的 efun：
...
- `find_player`
- `find_living`
- 其他项目内约定会返回固定对象类型的函数或 efun

配置型解析不是静态执行这些 efun，而是告诉扩展“这个函数在当前 mudlib 里可能返回哪些对象文件”。例如：
...
`instanceResolutionFunctions` 只适合返回单个 `object` 的函数。返回 `object *` 的 efun，例如 `users()`、`all_inventory()`、`children()`，当前不会被当成单个对象 receiver 解析；`users()[0]->method()`、`all_inventory()[0]->method()` 这类数组元素传播目前仍属于未覆盖场景。

已识别但默认无法静态求值的运行时 object efun：
...
这些 efun 未配置时会被标记为运行时动态来源，避免继续回退到 `@lpc-return-objects` 或其它静态猜测导致错误跳转。若其中某个返回单个 `object` 的 efun 在你的 mudlib 中有稳定对象类型，可以用 `instanceResolutionFunctions` 明确配置候选；返回 `object *` 的 efun 暂不支持配置为数组元素候选。
...
| `this_player()->method()` | `instanceResolutionFunctions.this_player` 中配置的候选对象 | |
| `environment(...)->method()` | `instanceResolutionFunctions.environment` 中配置的候选对象；未配置时标记为运行时动态 | |
| `master()->method()` | `config.hell` / `config.dev` 中的 `master file` | |
...
| 变量追踪 | 沿赋值链解析 | `ob = load_object("/obj/npc"); ob->method()` |
| `if/else` 合并 | 多候选 | 不同分支赋不同对象时合并为多候选 |
| 受限自然返回求值 | 静态可证明的函数返回对象 | `PROTOCOL_D->model_get("login")->error_result(...)` |
| 环境语义 provider | 配置驱动候选或运行时黑区分类 | `this_player()`、`previous_object(...)` |
| `@lpc-return-objects` | 自定义函数返回对象 fallback | 自然语义求值未知时继续传播 |
...
这些入口会尽量展示一致的函数说明：悬停与签名帮助会在当前文件、继承文件、include 文件、对象方法、模拟函数库和标准 efun 等来源之间查找合适的文档；当裸调用位置存在同名局部变量或函数参数时，会优先尊重语义作用域绑定，不再回退到 efun 文档；显式 `efun::method(...)` 会直接走标准 efun 文档。函数文档面板会展示当前文件、继承文件和 include 文件中的函数文档。
...
静态诊断依赖本地解析结果，覆盖语法错误、变量问题和部分语义问题。基础语义诊断会复用当前文件、include、inherit、标准 efun 与 simul-efun 中可确定的可见符号，提示直接调用参数数量、未定义函数和未定义普通符号；动态调用和无法完整解析的依赖会保守跳过。
...
- `this_player()` / `environment()` 无法解析：检查 `instanceResolutionFunctions`
