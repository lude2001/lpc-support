...
LPC Support 是一个面向 VS Code 的 LPC / FluffOS 开发扩展，当前版本为 `0.45.9`。
...
- 支持 `RequestType(...)` 这类宏展开体自带分号的顶层宏声明行，避免 HTTP controller 首次打开出现假语法诊断
- 修复 simulated efun 文档预加载后 `adm/simul_efun/message.c` 首次打开出现假的 `else` 语法诊断
...
语义高亮的主路径是 LSP semantic tokens，分类依据来自统一解析与语义摘要链路，而不是 TextMate 正则。TextMate grammar 只保留轻量词法兜底，用于文件刚打开、LSP 尚未返回或语言服务器不可用时提供基础颜色。

TextMate 不再维护 efun 大列表、函数名推断、变量名兜底或成员访问语义；这些 IDE 级语义统一交给 LSP。TextMate 只覆盖注释、字符串、预处理、基础关键字/类型、数字、操作符、heredoc 和 array delimiter。
...
3. 如果 `configHellPath` 对应文件存在，则解析其中的首批字段并自动回写到 `resolved`
4. 只有当解析结果发生变化时，才会重写 `lpc-support.json` 并刷新 `lastSyncedAt`
5. 如果 `config.hell` 缺失或解析失败，现有 `resolved` 会被保留，不会被清空

`resolved` 不是另一份独立手写配置，而是扩展从 `config.hell` 同步出来的结构化结果，也就是说当你键入configHellPath扩展会自动解析lib的config文件。

### `resolved` 当前会同步哪些字段

扩展当前从 `config.hell` 自动解析这些字段：
...
- `resolved` 和 `lastSyncedAt` 都由扩展维护，通常不建议手工编辑
- `include directories` 会按冒号分隔解析成数组
- `simulated efun file` 可以不带扩展名；扩展会按实际文件补全 `.c` / `.h`
- `resolved.mudlibDirectory` 如果是相对路径，会相对于 `configHellPath` 所在目录解析，而不是相对于工作区根目录
- `resolved.includeDirectories` 和 `resolved.simulatedEfunFile` 的最终实际路径，都会基于 `mudlibDirectory` 再做一次解析
