...
当前版本已经完成单一路径 LSP 收束。扩展的公开生产入口始终启动同一条 LSP client/server 运行链路，主语言能力链、诊断与格式化都通过这条路径提供。

同时，仍有少量宿主侧 affordances 保留在 VS Code 扩展宿主中，例如函数文档面板、错误树视图、编译管理与启动驱动命令。这些能力不再构成独立运行模式，但在排障时应按“LSP 主链 + host affordances”两层边界理解当前实现。

### 当前 LSP 能力状态

- 编辑体验主链已经统一到 LSP：`completion`、`hover`、`definition`、`references`、`rename`、`documentSymbol`、`foldingRange`、`semanticTokens`
- 深水区能力已经统一到 LSP：`diagnostics`、文档格式化、选区格式化
- server 运行时已经补齐 spawned integration、workspace/config sync、bundle 边界和单路径 cutover 回归
- 生产面不再公开多模式切换配置，文档与配置说明也已经同步到单一路径 LSP 口径

### 当前验证基线

- capability parity：8 项主语言能力共享 service 与 server handler 已锁定回归
- diagnostics parity：共享诊断服务、server publish path、host UX 分工已锁定
- formatting parity：共享 formatting service、LSP formatting handler、真实夹具输出已锁定
- runtime hardening：真实 `dist/lsp/server.js` spawned round-trip、多工作区 root 传播、Node-safe bundle 断言已锁定
...
当前版本只支持在项目根目录使用 `lpc-support.json` 作为项目级配置入口。
...
- 宏定义无法识别：检查 `lpc-support.json -> resolved.includeDirectories`
- 模拟函数库文档未生效：检查 `lpc-support.json -> resolved.simulatedEfunFile`
