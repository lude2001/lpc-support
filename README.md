...
从 `0.2.9` 开始，推荐在项目根目录使用 `lpc-support.json` 作为项目级配置入口，而不是手工维护 `lpc.includePath`、`lpc.simulatedEfunsPath` 等旧版设置。
...
- `resolved` 由扩展自动根据 `config.hell` 同步和回写
...
- `lpc-support.json -> compile.local.driverConfigPath`
