...
exports.ApplyFunctionReturnCollector = void 0;
const vscode = require("vscode");
const LPCParser_1 = require("../antlr/LPCParser");
/**
 * 检查 apply 函数的返回类型是否符合 LPC 规范
 */
class ApplyFunctionReturnCollector {
    constructor() {
        ...
        const visit = (ctx) => {
            if (ctx instanceof LPCParser_1.FunctionDefContext) {
                ...
                if (funcName && this.expectedReturnMap[funcName]) {
                    ...
                    const actual = typeCtx ? document.getText(new vscode.Range(document.positionAt(typeCtx.start.startIndex), document.positionAt((typeCtx.stop?.stopIndex ?? typeCtx.start.stopIndex) + 1))) : 'void'; // 默认为 void
                    if (actual !== expected) {
                        const range = new vscode.Range(document.positionAt(nameToken.startIndex), document.positionAt(nameToken.stopIndex + 1));
                        diagnostics.push(new vscode.Diagnostic(range, `apply 函数 '${funcName}' 返回类型应为 '${expected}'，当前为 '${actual}'`, vscode.DiagnosticSeverity.Warning));
                    }
                }
            }
...
exports.ApplyFunctionReturnCollector = ApplyFunctionReturnCollector;
//# sourceMappingURL=ApplyFunctionReturnCollector.js.map