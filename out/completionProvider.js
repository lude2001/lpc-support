...
class LPCCompletionItemProvider {
    constructor() {
        ...
        this.types = [
            ...
            'status', 'closure', 'symbol', 'pointer'
        ];
        // LPC 修饰符
        this.modifiers = [
            'private', 'public', 'protected', 'nosave', 'static',
            'nomask', 'varargs', 'virtual'
        ];
        // LPC 标准函数
        this.efuns = {
            // 数组相关函数（Arrays）
        ...
        this.snippets = {
            ...
            'add_action': {
                snippet: [
                    'void add_action(string verb) {',
                    '\tadd_action("do_" + verb, verb);',
                    '}',
                    '',
                    'int do_${1:verb}(string arg) {',
                    '\t$0',
                    '\treturn 1;',
                    '}'
                ].join('\n'),
    ...
    provideCompletionItems(document, position, token, context) {
        ...
        // 添加代码片段提示
        Object.entries(this.snippets).forEach(([name, info]) => {
