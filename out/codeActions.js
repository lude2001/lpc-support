...
class LPCCodeActionProvider {
    constructor() {
        if (LPCCodeActionProvider.commandsRegistered)
            return;
        const registerRenameCmd = (id) => {
            vscode.commands.registerCommand(id, async (uri, position, newName) => {
    ...
    provideCodeActions(document, range, context) {
        ...
        for (const diagnostic of context.diagnostics) {
            ...
            // === 未使用变量 / 参数 / 全局变量 ===
            if (diagCode === LPCCodeActionProvider.unusedVarDiagnosticCode ||
    ...
    createCommentVariableAction(document, diagnostic) {
        ...
        if (data && typeof data.start === 'number' && typeof data.end === 'number') {
            ...
            // ---- 向后扩展: 逗号 ----
            i = end;
            while (i < docText.length && /[ \t]/.test(docText[i]))
                i++;
            if (i < docText.length && docText[i] === ',') {
                end = i + 1; // 包含后逗号
            }
            const replaceRange = new vscode.Range(document.positionAt(start), document.positionAt(end));
     ...
     */
    createRenameVariableCaseAction(document, diagnostic, mode) {
     ...
     */
    createFixApplyReturnTypeAction(document, diagnostic) {
...
LPCCodeActionProvider.unusedParamDiagnosticCode = 'unusedParam';
LPCCodeActionProvider.unusedGlobalVarDiagnosticCode = 'unusedGlobalVar';
LPCCodeActionProvider.applyReturnMismatchDiagnosticCode = 'applyReturnMismatch';
LPCCodeActionProvider.commandsRegistered = false;
