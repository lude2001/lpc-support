...
export interface LpcContainerShapeEvaluatorOptions {
...
export class LpcContainerShapeEvaluator {
    public constructor(private readonly options: LpcContainerShapeEvaluatorOptions) {}
    ...
    public evaluateMappingLiteral(node: SyntaxNode, state: StaticEvaluationState): SemanticValue {
        ...
        for (const entryNode of node.children) {
            ...
            const keyValue = this.options.evaluateExpression(entryNode.children[0], state);
            const entryKey = literalValueToStaticKey(keyValue);
            if (entryKey === undefined) {
                return unknownValue();
            }

            entries[entryKey] = this.options.evaluateExpression(entryNode.children[1], state);
        }
    ...
    public evaluateArrayLiteral(node: SyntaxNode, state: StaticEvaluationState): SemanticValue {
        ...
        for (const child of expressionList.children) {
            ...
            elements.push(this.options.evaluateExpression(child, state));
        }
    ...
    public evaluateIndexExpression(node: SyntaxNode, state: StaticEvaluationState): SemanticValue {
        const target = this.options.evaluateExpression(node.children[0], state);
        const index = this.options.evaluateExpression(node.children[1], state);
