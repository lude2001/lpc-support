...
const lpcHighlightingQueries = {
    ...
    operators: `
      ...
      ; Add other specific operators if needed
    `,
...
**Brief Note on Integration in `src/extension.ts`:**

In the `activate` function of `src/extension.ts`, the `LPCSemanticTokensProvider` would be initialized and registered as follows:

```typescript
import * as vscode from 'vscode';
import * as Parser from 'web-tree-sitter';
import { LPCSemanticTokensProvider, legend } from './semanticHighlighter'; // Adjust path if needed

export async function activate(context: vscode.ExtensionContext) {
    // ... other extension activation code ...

    // Initialize Tree-sitter Parser and load the LPC grammar
    // This is a crucial step. The .wasm file for the grammar must be available.
    await Parser.init(); // Call this once
    const lpcGrammarWasmPath = vscode.Uri.joinPath(context.extensionUri, 'parser', 'tree-sitter-lpc.wasm').fsPath;
    try {
        const lpcLanguage = await Parser.Language.load(lpcGrammarWasmPath);
        LPCSemanticTokensProvider.setLanguage(lpcLanguage); // Pass the loaded language to the provider

        // Register the semantic tokens provider
        context.subscriptions.push(
            vscode.languages.registerDocumentSemanticTokensProvider(
                { language: 'lpc' }, // Assuming 'lpc' is the language ID defined in package.json
                new LPCSemanticTokensProvider(),
                legend
            )
        );
        console.log('LPC Semantic Tokens Provider registered.');

    } catch (error) {
        console.error('Failed to load LPC grammar for semantic highlighting:', error);
    }

    // ... other registrations like hover, completion providers ...
}
...
**Key points for the `extension.ts` integration:**
1.  **Language ID:** Ensure the language ID used in `registerDocumentSemanticTokensProvider` (e.g., `'lpc'`) matches the one defined in the `languages` section of your `package.json`.
2.  **Grammar Loading:** The `tree-sitter-lpc.wasm` file (generated from your `grammar.js`) must be correctly bundled with the extension (e.g., in a `parser` subdirectory) and its path provided to `Parser.Language.load()`. The `LPCSemanticTokensProvider.setLanguage(lpcLanguage)` static method is a way to make the loaded language available to the provider instances.
3.  **Error Handling:** Wrap grammar loading in a try-catch block as it's a common point of failure if the wasm file is missing or corrupt.
4.  **Activation:** Semantic highlighting is registered when the extension activates.

This setup provides the structure for AST-based semantic highlighting. The actual highlighting quality will depend on the accuracy of the Tree-sitter queries and the comprehensiveness of the `grammar.js` itself. The `provideDocumentSemanticTokens` method's token building logic might need refinement for multi-line tokens, but the core query-match-map-build loop is present.
