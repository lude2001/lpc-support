...
describe('preprocessor diagnostics regressions', () => {
    ...
    test('does not leak directive block comment continuations into parsed LPC code', () => {
        const document = createDocument([
            ...
            '',
            'void do_tests() {',
            '    mapping values = ([ COMMENT_STRING: 7 ]);',
            '    int UNDEF_ME = 9;',
            '',
            ...
            '}'
        ].join('\n'), '/virtual/compiler/preprocessor.c');

        const parsed = new ParsedDocumentService({ cleanupInterval: 0, enableMonitoring: true }).get(document);

        expect(parsed.diagnostics).toHaveLength(0);
        expect(parsed.parseText).not.toContain('and ends on the next physical line');
        expect(parsed.parseText).not.toContain('this text must stay skipped');
    });
});
